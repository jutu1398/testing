...
class ShuffleNetV2(nn.Module):
    ...
    def freeze_stages(self, stage):
        if stage >= 0:
            #self.bn1.eval()
            for m in [self.conv1, self.stage2]:
                for param in m.parameters():
                    param.requires_grad = False
        for i in range(2, stage + 1):
            layer = getattr(self, 'stage{}'.format(i))
            layer.eval()
...
def shufflenet_v2_x1_0(pretrained: bool = True, progress: bool = True, **kwargs: Any) -> ShuffleNetV2:
    """
